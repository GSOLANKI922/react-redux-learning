    ...
    CredentialsProvider({
      async authorize(credentials) {
        ...
        try {
          ...
          const { data, errors } =
            await serverAuthClient.mutate<LoginResponseData>({
              mutation: LOGIN,
              variables: {
                data: {
          ...
          if (!data?.emailPasswordLogIn?.data) {
            throw new AuthError("INVALID_CREDENTIAL", "Invalid credentials");
          }

          return {
            accessToken: data.emailPasswordLogIn.data?.token,
            refreshToken: data.emailPasswordLogIn.data?.refreshToken,
            user: data.emailPasswordLogIn?.data.user,
            id: data.emailPasswordLogIn?.data.user.id,
          };
        } catch (error) {
